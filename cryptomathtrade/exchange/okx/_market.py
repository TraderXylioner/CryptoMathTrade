...
from ._deserialization import _deserialize_depth, _serialize_trades, _serialize_ticker, _deserialize_symbols, \
    _deserialize_kline
...
class Market(API):
    def get_depth(self, symbol: str, limit: int = 1) -> Response[OrderBook, object]:
        """Get orderbook.
...
class AsyncMarket(API):
    async def get_depth(self, symbol: str, limit: int = 1) -> Response[OrderBook, object]:
        """Get orderbook.
    ...
    async def get_symbols(self, symbol: str = None) -> Response[list[Symbol], object]:
        ...
        params:
            symbol (str, optional): the trading pair
        """
        response = validate_response(await self._async_query(**MarketCore.get_symbols(self, symbol=symbol)))
        json_data = response.json
        return _deserialize_symbols(json_data, response)
