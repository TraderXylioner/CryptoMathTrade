...
from ._deserialization import _deserialize_balance, _deserialize_deposit_address, _deserialize_coins, \
    _deserialize_deposit_history, _deserialize_withdraw_history, _deserialize_withdraw
from .core import AccountCore
from .._response import Response
from ..utils import validate_response
from ...types import Balance, DepositAddress, Coin, DepositHistory, WithdrawHistory, Withdraw


class Account(API):
    def get_balance(self, asset: str = None) -> Response[list[Balance], object]:
        """Query Assets
        ...
        """
        response = validate_response(self._query(**AccountCore.get_balance(self, asset=asset)))
        json_data = response.json()
        return _deserialize_balance(json_data, response)

    def get_coins(self) -> Response[list[Coin], object]:
    ...
    def get_deposit_history(self,
        ...
        response = validate_response(self._query(
            **AccountCore.get_deposit_history(self, asset=asset, limit=limit, status=status, startTime=startTime,
                                              endTime=endTime, offset=offset, txId=txId)))
        json_data = response.json()
    ...
    def get_withdraw_history(self,
        ...
        """
        response = validate_response(self._query(
            **AccountCore.get_withdraw_history(self, asset=asset, limit=limit, withdrawOrderId=withdrawOrderId,
    ...
    def get_deposit_address(self,
        ...
        response = validate_response(
            self._query(**AccountCore.get_deposit_address(self, asset=asset, network=network, amount=amount)))
        json_data = response.json()
        return _deserialize_deposit_address(json_data, response)


class AsyncAccount(API):
    async def get_balance(self, asset: str = None) -> Response[list[Balance], object]:
        """Query Assets
        ...
        params:
            asset (int, optional): If asset is blank, then query all positive assets user have.
        """
        response = validate_response(await self._async_query(**AccountCore.get_balance(self, asset=asset)))
        json_data = response.json
        return _deserialize_balance(json_data, response)

    async def get_coins(self) -> Response[list[Coin], object]:
        """All Coins' Information
        ...
        https://binance-docs.github.io/apidocs/spot/en/#all-coins-39-information-user_data
        """
        response = validate_response(await self._async_query(**AccountCore.get_coins(self)))
        json_data = response.json
        return _deserialize_coins(json_data, response)
    ...
    async def get_deposit_address(self,
        ...
        """
        response = validate_response(await self._async_query(
            **AccountCore.get_deposit_address(self, asset=asset, network=network, amount=amount)))
        json_data = response.json
        return _deserialize_deposit_address(json_data, response)
