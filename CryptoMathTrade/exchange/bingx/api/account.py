...
from ..core.account import AccountCore, WSAccountCore
...
class Account(API):
    ...
    def generate_listen_key(self):
        ...
        """
        response = validate_response(
            self._query(**AccountCore.generate_listen_key(self))
        )
        json_data = response.json()
        return deserialize.deserialize_listen_key(json_data, response)
        ...
        response = validate_response(
            self._query(
                **AccountCore.get_deposit_address(
                    self, asset=asset, limit=limit, offset=offset
                )
            )
        )
        json_data = response.json()
...
class AsyncAccount(API):
    ...
    async def generate_listen_key(self):
        ...
        """
        response = validate_response(
            await self._async_query(**AccountCore.generate_listen_key(self))
        )
        json_data = response.json
        return deserialize.deserialize_listen_key(json_data, response)
    ...
    async def get_balance(self):
        ...
        """
        response = validate_response(
            await self._async_query(**AccountCore.get_balance(self))
        )
        json_data = response.json
        return deserialize.deserialize_balance(json_data, response)
...
class WebSocketAccount(API):
    async def account_update(self, listenKey: str):
        ...
        async for response in self._ws_query(
            **WSAccountCore.account_update(self, listenKey=listenKey)
        ):
            if time.time() >= time_end:
                break
            json_data = json.loads(
                gzip.GzipFile(fileobj=io.BytesIO(response), mode="rb").read().decode()
            )
            if "a" in json_data:
                yield deserialize.deserialize_account_update_for_ws(json_data, response)
